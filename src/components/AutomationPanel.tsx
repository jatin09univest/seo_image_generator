...
import { Zap, CheckCircle, XCircle, AlertCircle, Loader2, ExternalLink } from "lucide-react";
import type { ConfigStatus, AutomationRunResult, RowResult } from "@/lib/automation-types";
...
export default function AutomationPanel() {
  ...
  const [spreadsheetInput, setSpreadsheetInput] = useState("1w3MuuCkd1PV5DTMoDuYOvnfm8hiyqhSOldQvfIIxrrQ");
    ...
    <div className="space-y-5">
      ...
      {config && (
        <div
          ...
          style={{
            background: config.ok ? "rgba(34,197,94,0.08)" : "rgba(251,191,36,0.08)",
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <p className="text-[10px]" style={{ color: "var(--text-muted)" }}>
            Parsed ID: <code>{extractSpreadsheetId(spreadsheetInput.trim()) || "—"}</code>
          </p>
        </div>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            placeholder={config?.configured.defaultFolderId ? "Using default from env" : "Enter folder ID"}
            className="w-full text-xs px-3 py-2 rounded-lg outline-none focus:ring-1 focus:ring-blue-500"
            style={{
              background: "var(--bg-elevated)",
              border: "1px solid var(--border)",
          ...
          {config?.configured.defaultFolderId && !driveFolderInput && (
            <p className="text-[10px]" style={{ color: "var(--text-muted)" }}>Using default folder from environment</p>
          )}
        </div>
          ...
          <div className="flex items-center gap-1">
            {[1, 2, 3, 4, 5].map(n => (
              <button
                ...
                style={{
                  background: batchSize === n ? "rgba(37,99,235,0.2)" : "var(--bg-elevated)",
                  border: batchSize === n ? "1px solid rgba(37,99,235,0.5)" : "1px solid var(--border)",
        ...
        >
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-3 text-xs">
              <span style={{ color: "var(--text-secondary)" }}>{result.processed} processed</span>
              <span style={{ color: "#22c55e" }}>{result.succeeded} succeeded</span>
            ...
            <table className="w-full text-xs" style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid var(--border)" }}>
                  {["Row", "Company", "Template", "Status", "Image"].map(h => (
                    <th key={h} className="text-left py-2 pr-4 font-semibold" style={{ color: "var(--text-muted)" }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {result.results.map((r: RowResult) => (
                  <tr key={r.rowIndex} style={{ borderBottom: "1px solid var(--border)" }}>
                    <td className="py-2 pr-4" style={{ color: "var(--text-muted)" }}>{r.rowIndex}</td>
                    <td className="py-2 pr-4 font-medium" style={{ color: "var(--text-primary)" }}>{r.companyName}</td>
                    <td className="py-2 pr-4" style={{ color: "var(--text-secondary)" }}>{r.templateType}</td>
                    <td className="py-2 pr-4">
                      {r.status === "success"
                        ? <span className="px-2 py-0.5 rounded-full font-medium" style={{ background: "rgba(34,197,94,0.12)", color: "#22c55e" }}>Done</span>
                        : <span className="px-2 py-0.5 rounded-full font-medium" style={{ background: "rgba(239,68,68,0.12)", color: "#ef4444" }}>Error</span>
                      }
                    </td>
                    <td className="py-2 pr-4">
                      {r.imageUrl
...
function ConfigFlag({ label, ok }: { label: string; ok: boolean }) {
  return (
    <span className="flex items-center gap-1" style={{ color: ok ? "#22c55e" : "#f87171" }}>
      {ok ? "✓" : "✗"} {label}
    </span>
  );
}
