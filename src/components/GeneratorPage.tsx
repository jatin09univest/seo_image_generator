...
function getCompanyName(values: Record<string, string>): string {
  return (
    values.companyName || values.stockName || values.headlineText || "Unknown"
  );
}
...
export default function GeneratorPage() {
  ...
  const [contract, setContract] = useState<
    import("@/lib/types").ContractResult | null
  >(null);
  ...
  const [activeTab, setActiveTab] = useState<"generator" | "automation">(
    "generator",
  );
  ...
  const saveToHistory = useCallback(
    async (imageSource: string, usedPrompt: string) => {
      if (!selectedTemplate) return;
      const companyName =
        fieldValues.companyName ||
        fieldValues.stockName ||
        fieldValues.headlineText ||
        "Unknown";
      const isUrl = imageSource.startsWith("http");
      const imageUrl = isUrl
        ? imageSource
        : await compressImageForStorage(imageSource);
  ...
  const handleDownloadAllZip = useCallback(async () => {
    const readyVariants = bulkHook.variants.filter(
      (variant): variant is BulkVariantResult & { url: string } =>
        !!variant.url,
    );
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.BASE_URL}download-variants-zip`,
        {
          method: "POST",
  ...
  const handleUploadZipToDrive = useCallback(async () => {
    ...
    try {
      const companyName = sanitizeFileName(getCompanyName(fieldValues));
      const response = await fetch(
        `${process.env.BASE_URL}upload-variants-zip`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!response.ok || !data.success) {
        throw new Error(
          data.error || `Failed to upload ZIP (${response.status})`,
        );
      }
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span
                className="text-sm ml-1.5 font-normal"
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              style={{
                color: cacheCleared ? "#34d399" : "var(--text-muted)",
                border: "1px solid var(--border)",
              }}
            >
        ...
        {activeTab === "generator" && (
          <>
                ...
                {template && (
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    >
                      ...
                      {genHook.loading
                        ? `Generating${genHook.retryCount > 0 ? ` (retry ${genHook.retryCount}/3)` : "..."}`
                        : "Generate Image"}
                    </button>
            ...
            {showVariants && bulkHook.variants.length > 0 && (
              <section
                className="mt-6 rounded-2xl p-4 sm:p-5 animate-fade-in"
                style={{
                  background: "var(--bg-card)",
                  border: "1px solid var(--border)",
                }}
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    {/* Download All */}
                    {bulkHook.variants.some((v) => v.url) &&
                      !bulkHook.isGenerating && (
                ...
                <div
                  className={`grid gap-4 ${getGridCols(bulkHook.variants.length)}`}
                >
